...
$('addCardBtn').addEventListener('click', ()=>{
  ...
  if(!term || !answer) 
    return alert('Заполните слово и значение');
  const d = getActiveDeck();
...
$('addExampleInCard').addEventListener('click', () =>{
...
$('exportBtn').addEventListener('click', ()=>{
  ...
  a.href = url; 
  a.download = 'educards_export.json'; 
  a.click(); 
...
$('fileInput').addEventListener('change', async (e)=>{
  ...
  { 
    ...
    decks = decks.concat(imported.map(normalizeDeck)); 
    saveDecks(); 
    renderDecksList(); 
    alert('Импортировано'); 
  }
...
function loadDecks(){ 
  ...
  if(!raw) 
    return sampleDecks(); 
  return JSON.parse(raw).map(normalizeDeck); 
}
catch(e)
{
  return sampleDecks()
} }
function normalizeDeck(d){ return {...{id:'d_'+Date.now(),
  title:'Без названия',
  description:'',
  visibility:'private',
  tags:[],
...
function renderDecksList(filter=''){
  ...
  decksList.innerHTML='';
  if(!decks.length){ 
    decksList.innerHTML='<div class="muted">Колод нет</div>'; 
    return 
  }
  decks.filter(d=>{
    if(!q) return true;
    ...
    el.innerHTML = `<div class="meta">
      ...
      <div class="muted">
        ${escape(d.tags?d.tags.join(', '):'')}
        </div>
    </div>
    <div class="muted small">${d.cards.length} карточек</div>`;
...
function renderActive(){
  if(!activeDeckId){ deckEditor.style.display='none'; emptyState.style.display='block'; 
    deckTitle.textContent='Выберите колоду или создайте новую'; 
  ...
  if(!d.cards.length)
    { 
      grid.innerHTML='<div class="muted">Пока нет карточек</div>'; 
      return 
    }
  d.cards.forEach(c=>{
    const ci = document.createElement('div'); 
    ci.className='card-item';
    ci.innerHTML = `<strong>${escape(c.term)}</strong>
    <div class="muted small" style="margin-top:6px">
      ${escape(c.answer)}
    </div>
    <div style="margin-top:8px" class="flex">
      <button class="ghost" data-id="${c.id}">Ред.</button>
      <button class="ghost" data-del="${c.id}">Удал.</button>
    </div>`;
    grid.appendChild(ci);
  });
  ...
  grid.querySelectorAll('[data-id]').forEach(b => b.addEventListener('click', e=>{
    ...
    if(!card) return; 
    const t = prompt('Термин', card.term); 
    if(t===null) return; 
    const a = prompt('Значение', card.answer); 
    if(a===null) return; 
    card.term=t; 
    card.answer=a; 
    d.updated_at=now(); 
...
function sampleDecks(){ return [
  {
    id:'d_sample_med', 
    title:'Medical English — Anatomy', 
    description:'Базовые термины по анатомии', 
    visibility:'public', 
...

  {
    id:'d_sample_it', 
    title:'IT — Networking', 
    description:'Основные термины сетей', 
    visibility:'public', 
    tags:['it','networking'], 
    cards:[
      {id:'c3',term:'router',answer:'роутер'},
      {id:'c4',term:'switch',answer:'коммутатор'}], 
    created_at:now(), 
    updated_at:now()}
]}

// Check URL param (share link)
(function checkShare(){ const params = new URLSearchParams(location.search); 
  if(params.has('deck')){ 
    try
    { 
      const blob = decodeURIComponent(params.get('deck')); 
      const json = decodeURIComponent(escape(atob(blob))); const deck = JSON.parse(json); 
      deck.id = 'd_import_'+Date.now(); 
      decks.unshift(normalizeDeck(deck)); 
      saveDecks(); 
      renderDecksList(); 
      alert('Импортирована колода из ссылки'); 
    }
    catch(e)
    {
      console.warn(e)
